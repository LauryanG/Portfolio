let projets = [
    {
        titre: "Bo Dollar",
        description: "Projet dans le cadre du cours de design d'interface qui avait pour but de refaire le design d'un site web.",
        images: ["media/media-projets/boDollar/Accueil.jpg", "media/media-projets/boDollar/Magasin.jpg", "media/media-projets/boDollar/Produits.jpg"],
        video: false,
        videos: [""]

    },
    {
        titre: "Bracelet 3D",
        description: "Première création 3D avec le logiciel Maya. Le projet consistait à imaginer un objet de tous les jours le modéliser et lui ajouter des textures.",
        images: ["media/media-projets/bracelet/Bracelet_Haut.jpg", "media/media-projets/bracelet/Bracelet_profilBas.jpg", "media/media-projets/bracelet/Bracelet_profilHaut.jpg"],
        video: true,
        videos: ["media/media-projets/bracelet/GERVAIS_Lauryan_TP1.mp4"]
    },
    {
        titre: "Dark Planet",
        description: "Dark Planet est un projet d'équipe de jeu vidéo. Il s'agit d'un prototype de jeu 3D d'espionnage futuriste et post apocalyptique qui nous met dans la peau d’un alien qui s’écrase avec son vaisseau sur terre. Le but est simple, aller chercher les pièces manquantes du vaisseau et s'enfuir de la planète. <br> <br> <a href=\"https://getpranked-ori.itch.io/dark-planet \" target=\"_blank\"> Jouer à Dark Planet</a>",
        images: ["media/media-projets/darkPlanet/AlienLigght.jpg", "media/media-projets/darkPlanet/Bunker_1.jpg", "media/media-projets/darkPlanet/cultisteCouple.jpg"],
        video: true,
        videos: ["https://www.youtube.com/watch?v=9DfjePIRikU"]

    },
    {
        titre: "Dirigeable steampunk 3D",
        description: "Le dirigeable de style fantaisie steampunk est un projet 3D pour apprendre à faire des véhicules.",
        images: ["media/media-projets/dirigeable/Gervais_Lauryan_affiche.jpg", "media/media-projets/dirigeable/GervaisLauryan_1.jpg", "media/media-projets/dirigeable/GervaisLauryan_2.jpg", "media/media-projets/dirigeable/GervaisLauryan_4.jpg"],
        video: true,
        videos: ["media/media-projets/dirigeable/Gervais_Lauryan_TP2.mp4"]
    },
    {
        titre: "Design de site de restaurant",
        description: "Maquette design pour un site web de restaurant en one pager.",
        images: ["media/media-projets/carteBlanche/accueil_carte.PNG", "media/media-projets/carteBlanche/a_propos_carte.PNG", "media/media-projets/carteBlanche/apropos_carte.PNG", "media/media-projets/carteBlanche/contact_carte.PNG"],
        video: false,
        videos: [""]
    },
    {
        titre: "TIM 2020",
        description: "Projet d'équipe de développement web. Le projet consiste à repenser le design et programmer un nouveau site départemental pour la technique d'intégration multimédia du collège de Maisonneuve.",
        images: ["media/media-projets/tim/accueil_tim.PNG", "media/media-projets/tim/cours_tim.PNG", "media/media-projets/tim/prof_tim.PNG", "media/media-projets/tim/vie_tim.PNG", "media/media-projets/tim/stage_tim.PNG", "media/media-projets/tim/evenement_tim.PNG"],
        video: false,
        videos: [""]
    },
    {
        titre: "Les mots",
        description: "Création du design et programmation d'un jeu avec la librairie Phaser. Il s'agit d'un jeu éducatif dont le but est de trouver le plus de mot possible avec les lettres données en moins de 60 secondes.",
        images: ["media/media-projets/les mots/accueil.PNG","media/media-projets/les mots/jouer.PNG", "media/media-projets/les mots/jeu.PNG"],
        video: false,
        videos: [""]
    },
    {
        titre: "Logo",
        description: "Projet de création de mon logo. Le logo a été créer avec le logiciel Adobe Illustrator en hiver 2019 dans un cours de design. Le logo a ensuite été retravaillé en février 2021 comme projet personnel.",
        images: ["media/media-projets/logo/Logo-final-bleu.png", "media/media-projets/logo/Logo-final-mauve.png", "media/media-projets/logo/logo_bleu.png", "media/media-projets/logo/logo_mauve.png"],
        video: false,
        videos: [""]
    },
    {
        titre: "Mustache Man",
        description: "Création d'un jeu vidéo en 2D. Il s'agit d'un jeu de plateforme ou le joueur a pour but d'accumuler le plus de point en amassant des pièces sans mourir. Plus le joueur à des points, plus le jeu devient difficile.",
        images: ["media/media-projets/mustache man/Gervais_Affiche_TP2B.png", "media/media-projets/mustache man/Gervais_Splash_screen_TP2B_HUD.png", "media/media-projets/mustache man/Gervais_Splash_screen_TP2B_Instruction.png", "media/media-projets/mustache man/Gervais_Splash_screen_TP2B_Intro.png", "media/media-projets/mustache man/Gervais_Splash_screen_TP2B_Perdu.png"],
        video: false,
        videos: [""]
    }
]
